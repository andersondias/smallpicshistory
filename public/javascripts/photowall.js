$(function() {
  function renderDetails(image, detailsDiv) {
    $('#photo-title').html(image.attr('title'));
    $.each(['description', 'when', 'where'], function(index, attrName) { 
      var attrId = '#photo-' + attrName;
      var attrValue = detailsDiv.find('.pic-' + attrName).html();

      if(!$.isEmptyObject($.trim(attrValue))) {
        $(attrId).html(attrValue).show();
        $(attrId + '-label').show();
      } else {
        $(attrId).hide();
        $(attrId + '-label').hide();
  ...
  $("#thumbs > img").click(function() {
    $('#sample-image').attr('src', $(this).attr('alt'));
    renderDetails($(this), $(this).next());

    $.fancybox($('#wrapper').html(), {
  ...
  //*/
  //$('#photo-image-panel > img').live('click',function(){
  //  $this = $(this);
  //  $('#description').empty().hide();
  //
  ...
  //  $('#photo-image-panel').css({'width':theImage.width+'px','height':theImage.height+'px'});
  //}
});
